...
def dechargesInformationsEnvoi(request): 
        ...
        geolocalisation= request.data['location']
       ...
       # geolocalisation_accuracy = request.data['location'].coords.accuracy
        ...
        serializer = DechargeSerializer(data={'localisation_decharge': localisation_decharge,'description_decharge' : description_decharge,'dimension_decharge' : dimensions_decharge,'dechets_observes':dechets_observes,'nuisances_observees' : nuisances_observees,'description_situation' : description_situation,'observation' : observation,'photo_decharge' : photo_decharge,'geolocalisation_longitude' : geolocalisation.coords.longitude,'geolocalisation_latitude':geolocalisation.coords.latitude,'geolocalisation_altitude' : geolocalisation.coords.altitude, 'geolocalisation_altitudeAccuracy' : geolocalisation.coords.altitudeAccuracy,'geolocalisation_accuracy':geolocalisation.coords.accuracy,'geolocalisation_speed': geolocalisation.coords.speed,'data_operation':str(datetime.now().date()),'date_heure_operation':str(datetime.now()),'month_year_operation':str(datetime.now().year)+"-"+"0"+str(datetime.now().month)})
